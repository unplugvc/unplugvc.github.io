...
$(document).ready(function () {
  ...
  var scheduleMenuWidth = $('.schedule__line-date-element-sizer--mobile li').length * 88 + 100;
...
$(document).scroll(function () {
  ...
  if (!state.menu_voices_scrolltop.length && $('#about').length) {
    state.menu_voices_scrolltop = [['#about', $('#about').offset().top], ['#schedule', $('#schedule').offset().top], ['#past_editions', $('#past_editions').offset().top], ['#contacts', $('#contacts').offset().top]];
  }

  if ($('#about').length) {
    var current_id = 0;
    for (var i = 0; i < state.menu_voices_scrolltop.length; i++) {
      var scrollTop = state.menu_voices_scrolltop[i][1] - 160;
      if (i == 1) {
    ...
    $('.top_navbar__menu a').eq(current_id).css({ borderBottom: '3px solid #9d774f' });
    if ($(document).scrollTop() < state.menu_voices_scrolltop[0][1] - 50) {
      $('.top_navbar__menu a').eq(0).css({ borderBottom: '0px' });
    }
  }
});
...
function displayScheduleTab(id) {
  ...
  $('html,body').stop(true);
  if (ww < 769) {
    var nextPosition = $('.schedule__tabs').offset().top - 200;
...
function negativeMenuToggle() {
  ...
  if (ww < 769) {
    ...
    if ($('.schedule__line-container').length) {
      var menuScrolltop = $('.schedule__line-container').offset().top - 76;
...
var lazy_backgrounds = debounce(function () {
...
function playVideo() {
  if (document.getElementById('event_video')) {
    var vid = document.getElementById('event_video');
    var _wscroll = $(window).scrollTop();
    if ($('#event_video').offset().top <= _wscroll + (wh + 100)) {
      vid.play();
    }
  }
}
